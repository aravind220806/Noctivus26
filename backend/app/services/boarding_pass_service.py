...
.main {{ min-width: 0; position: relative; display: grid; grid-template-rows: 122px 95px 90px 84px 156px 33px; background: linear-gradient(180deg,#fff 0%,#f7fbff 100%); }}
.stub {{ min-width: 0; border-left: 2px dashed #9AA6B8; display: grid; grid-template-rows: 58px 1fr 72px; background: #fff; position: relative; }}
...
.header {{ display: grid; grid-template-columns: 260px 1fr 330px; align-items: center; padding: 18px 46px 16px 74px; gap: 10px; }}
.logo {{ width: 94px; height: 94px; object-fit: contain; }}
.brand h1 {{ margin: 0; font: 800 78px/0.88 "Roboto Condensed",Arial,sans-serif; letter-spacing: 0; }} .brand h1 span {{ color:#356AE6; }}
.brand p {{ margin: 12px 0 0; color: #fff; font: 700 18px/1 Inter,Arial,sans-serif; letter-spacing: 11px; }}
.pass-title {{ justify-self: end; display: flex; align-items: center; gap: 22px; font: 800 30px/1 "Roboto Condensed"; letter-spacing: 1px; }}
.plane {{ font-size: 48px; color: #dbe8ff; transform: rotate(8deg); }}
.row {{ min-width: 0; display: grid; align-items: start; column-gap: 0; padding: 20px 46px 0 122px; }}
.row1,.row2 {{ padding-right: 286px; }}
.row1 {{ grid-template-columns: 37% 43% 20%; border-bottom: 1px solid #D7DEE8; }}
.row2 {{ grid-template-columns: 15% 15% 45% 25%; border-bottom: 1px solid #D7DEE8; }}
.row2 .field {{ padding-left: 18px; padding-right: 18px; }}
.row3 {{ grid-template-columns: 12% 12% 10% 27% 39%; border-bottom: 1px solid #D7DEE8; }}
.field {{ min-width: 0; min-height: 48px; padding: 0 26px; border-left: 1px solid #AAB6C8; }}
.field:first-child {{ border-left: 0; padding-left: 0; }}
.label {{ display:block; color:#194FD1; font:800 13px/1 Inter; letter-spacing:.5px; text-transform:uppercase; margin-bottom:10px; }}
...
.main-value {{ font-size: 27px; }} .small-value {{ font-size: 25px; }} .email-value {{ font-size: 19px; }}
.qr-cell {{ position:absolute; right:46px; top:138px; padding:0; }} .qr-cell img {{ width: 184px; height: 184px; border: 3px solid #111827; border-radius: 10px; padding: 9px; background:#fff; }}
.route-wrap {{ padding: 18px 46px 0 78px; }}
.route-box {{ min-height: 136px; height:136px; display:grid; grid-template-columns:40% 20% 40%; align-items:center; background:#EAF2FF; border-radius:12px; padding: 16px 28px; overflow:hidden; }}
...
.route-center {{ display:grid; place-items:center; color:#194FD1; font-size:34px; overflow:hidden; }} .dotted {{ width:100%; border-top:3px dotted #194FD1; position:relative; }} .dotted span {{ position:absolute; left:50%; top:-23px; transform:translateX(-50%); background:#EAF2FF; padding:0 12px; }}
.to-block {{ justify-self:end; width:100%; padding-right: 4px; }} .to-block .college-line {{ font-size:25px; }}
.footer {{ text-align:center; color:#111827; font:700 14px/1 Inter; letter-spacing:12px; padding-top:8px; }}
.footer b {{ color:#194FD1; }}
.stub-head {{ display:grid; place-items:center; font:800 28px/1 "Roboto Condensed"; letter-spacing:1px; }}
.stub-body {{ padding: 17px 30px 0 34px; display:grid; grid-template-rows:auto auto auto auto auto minmax(70px,auto) auto; gap:8px; min-width:0; }}
.stub-field {{ min-width:0; min-height:43px; border-bottom:1px solid #D7DEE8; padding-bottom:7px; }}
.stub-field .value {{ font-size:22px; }} .stub-grid2 {{ display:grid; grid-template-columns:1fr 1fr; gap:12px; }} .stub-grid3 {{ display:grid; grid-template-columns:1fr 1fr 1fr; gap:12px; }}
.stub-grid2 .stub-field,.stub-grid3 .stub-field {{ border-left:1px solid #AAB6C8; padding-left:14px; }} .stub-grid2 .stub-field:first-child,.stub-grid3 .stub-field:first-child {{ border-left:0; padding-left:0; }}
.stub-college {{ min-height:78px; }} .stub-college .value {{ font-size:17px; -webkit-line-clamp:3; }}
.stub-to .value {{ font-size:18px; }}
.stub-foot {{ display:grid; grid-template-columns:74px 1px 1fr; align-items:center; gap:18px; padding:9px 28px; }} .stub-foot img {{ width:58px; height:58px; object-fit:contain; }} .stub-foot i {{ height:44px; background:#D7DEE8; }} .stub-foot strong {{ font:800 30px/1 "Roboto Condensed"; }} .stub-foot span {{ color:#356AE6; }} .stub-foot small {{ display:block; margin-top:4px; font:700 10px/1 Inter; letter-spacing:4px; }}
</style>
</head>
<body>
<div class="ticket">
  <main class="main">
    ...
    <section class="row row2"><div class="field"><span class="label">Time</span><span class="value small-value">{values['EVENT_TIME']}</span></div><div class="field"><span class="label">Gate</span><span class="value small-value two-line-field" data-autofit data-max-font="25" data-min-font="13" data-max-lines="2">{values['EVENT_GATE']}</span></div><div class="field"><span class="label">Venue</span><span class="value small-value two-line-field" data-autofit data-max-font="25" data-min-font="13" data-max-lines="2">{values['VENUE']}</span></div><div class="field"><span class="label">Terminal</span><span class="value small-value two-line-field" data-autofit data-max-font="25" data-min-font="13" data-max-lines="2">{values['TERMINAL']}</span></div></section>
    ...
    <div class="footer">IGNITE <b>•</b> INNOVATE <b>•</b> INSPIRE</div>
  </main>
