...
def pass_values(registration: dict, pass_data: dict) -> dict[str, str]:
    ...
    if not event_1_name and events:
        ...
        time_1 = time_1 or _format_time_display(pass_data.get("time") or events[0].get("batchTime") or events[0].get("time") or "10:00 AM")
        if len(events) > 1:
    ...
    if not event_1_name:
        ...
        time_1 = time_1 or _format_time_display(pass_data.get("time") or "10:00 AM")
...
.main {{ min-width: 0; position: relative; display: grid; grid-template-rows: 116px 92px 88px 92px 142px 50px; background: linear-gradient(180deg,#fff 0%,#f7fbff 100%); }}
...
.brand {{ min-width: 0; }}
.brand h1 {{ margin: 0; font: 800 44px/1 "Roboto Condensed",Arial,sans-serif; letter-spacing: 0; white-space: nowrap; }} .brand h1 span {{ color:#356AE6; }}
.brand p {{ margin: 9px 0 0; color: #fff; font: 700 16px/1 Inter,Arial,sans-serif; letter-spacing: 5px; white-space: nowrap; }}
...
.value {{ display:block; font-family:Arial,sans-serif; font-size:20px; font-weight:700; color:#111827; line-height:1.15; max-width:100%; overflow-wrap:anywhere; word-break:normal; hyphens:auto; overflow:hidden; }}
.two-line-field {{ display:block; }}
.main-value,.small-value,.email-value {{ font-size:20px; }}
...
.route-box {{ min-height: 118px; height:118px; display:grid; grid-template-columns:40% 20% 40%; align-items:center; background:#EAF2FF; border-radius:12px; padding: 16px 28px; overflow:hidden; }}
.route-label {{ color:#194FD1; font:700 14px/1.2 Arial,sans-serif; letter-spacing:.5px; text-transform:uppercase; margin-bottom:9px; }}
.college-line {{ font:700 20px/1.15 Arial,sans-serif; }} .hint {{ display:block; margin-top:7px; font:700 14px/1 Inter; color:#667085; }}
...
.to-block {{ justify-self:end; width:100%; padding-right: 4px; }} .to-block .college-line {{ font-size:20px; }}
...
.stub-field .label {{ font-size:14px; margin-bottom:4px; }}
.stub-field .value {{ font-size:20px; line-height:1.15; }} .stub-grid2 {{ display:grid; grid-template-columns:1fr 1fr; gap:8px; }}
...
.stub-college {{ min-height:56px; }} .stub-college .value {{ font-size:20px; }}
.stub-to {{ min-height:48px; }} .stub-to .value {{ font-size:20px; }}
