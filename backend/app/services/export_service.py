...
def export_attendance_to_excel(events: list[dict], registrations: list[dict]) -> bytes:
    ...
    sub_header_font = Font(name="Calibri", size=10, bold=True, color="E2E8F0")

    present_fill = PatternFill(start_color="DCFCE7", end_color="DCFCE7", fill_type="solid") # light green
    present_font = Font(name="Calibri", size=10, bold=True, color="166534") # dark green
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # Helper to extract all members for a registration and specific event
    # ══════════════════════════════════════════════════════════════════════════
    def get_event_members(reg: dict, target_event_id: str) -> list[dict]:
        ...
        # Check attendance info in eventRegistration or top-level attendance
        ...
        # 1. Team Leader (Main Participant)
    ...
    # Iterate through each event and collect member data
    for s_idx, ev in enumerate(events, 1):
        ...
        # Collect all registrations registered for this event
        event_regs = [
        ...
        # Sort: Present first, then Registration ID, then Role
        ...
        for m_idx, m in enumerate(sorted_event_members, 1):
            ...
            for col_idx in range(1, len(row_data) + 1):
                ...
                # Color highlight for Attendance & E-Cert status
                if col_idx in (13, 14):
