...
def sign_admin_token(user: dict) -> tuple[str, str]:
    ...
    payload = _b64encode(
        json.dumps(
            {**user, "exp": int(time.time() * 1000) + SESSION_TTL_MS, "csrf": csrf},
            separators=(",", ":"),
        ).encode()
    )
...
async def require_admin(
    ...
    return await _resolve_admin(request, authorization, noctivus_admin_session)
...
def require_admin_tab(tab: str) -> Callable:
    async def dependency(
        ...
        admin = await _resolve_admin(request, authorization, noctivus_admin_session)
