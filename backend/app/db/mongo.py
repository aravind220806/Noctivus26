...
async def connect_mongo() -> None:
    ...
    try:
        ...
        await db.registrations.create_index([("invitation.qrHash", ASCENDING)])
        await db.registrations.create_index([("invitation.qrToken", ASCENDING)])
        await db.registrations.create_index([("foodClaimed", ASCENDING)])
        await db.admin_access.create_index([("email", ASCENDING)], unique=True)
        await db.admin_access.create_index([("updatedAt", DESCENDING)])
        await db.admin_sessions.create_index([("sid", ASCENDING)], unique=True)
        await db.admin_sessions.create_index([("email", ASCENDING)])
        await db.admin_sessions.create_index([("expiresAt", ASCENDING)], expireAfterSeconds=0)
