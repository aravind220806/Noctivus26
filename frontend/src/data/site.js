export const site = {
  eyebrow: 'Department of CSE (Cyber Security) presents',
  date: '26 September 2026',
  eventStart: '2026-09-26T09:00:00+05:30',
  contactEmail: 'noctivus26@velammal.edu.in',
  contactPhone: '+91 98765 43210',
  address: 'Velammal Engineering College, Ambattur-Red Hills Road, Surapet, Chennai, Tamil Nadu 600066',
  coordinates: {
    latitude: 13.1483288,
    longitude: 80.1916095,
  },
  social: {
    Instagram: 'https://www.instagram.com/noctivus_25',
    LinkedIn: 'https://www.linkedin.com/company/noctivus-symposium/',
    X: 'https://x.com/noctivussympo',
  },
};

const eventImage = '/images/noctivus-students.webp';

export const events = [
  {
    id: 'crime-investigation',
    name: 'Crime Investigation',
    category: 'Non-technical',
    teamSize: '2–4 Members',
    fee: 150,
    accent: 'lime',
    image: eventImage,
    imagePosition: '50% 42%',
    format: 'Solve a crime case using the clues and evidence given by the organizers. Complete the investigation within the given time.',
    details: [
      'Use only the clues and evidence provided.',
      'Do not use mobile phones, internet, or AI tools.',
      'Do not remove, change, or exchange evidence.',
      'Do not communicate with other teams.',
      'Cheating will lead to disqualification.',
      'Submit the final answer before time ends.',
      'A tie-breaker may be conducted if needed.',
      "Judges' decision is final.",
    ],
    coordinators: [
      { name: 'Madhumitha A', phone: '9788339084' },
      { name: 'Harish Kumar S', phone: '9150543420' },
    ],
  },
  {
    id: 'bug-bounty-2026',
    name: 'Bug Bounty 2026',
    category: 'Technical',
    teamSize: 'Individual',
    fee: 200,
    accent: 'cyan',
    image: eventImage,
    imagePosition: '58% 45%',
    format: 'Find and report security vulnerabilities through live, hands-on cybersecurity challenges. Covers web security, infrastructure security, authentication, authorization, privilege escalation, and exploitation.',
    details: [
      'Test only the targets given by the organizers.',
      'Use ethical and legal methods only.',
      'Manual security testing is allowed.',
      'AI assistants can be used for learning, reasoning, and documentation.',
      'AI agents and autonomous tools must not interact with the challenge system.',
      'Do not use automated tools to attack or scan the infrastructure.',
      'Do not perform DoS/DDoS attacks.',
      'Do not access systems outside the allowed scope.',
      'Every vulnerability must include a valid PoC and explanation.',
      'Breaking the rules may lead to disqualification.',
    ],
    coordinators: [
      { name: 'Madhumitha T', phone: '7200291695' },
      { name: 'Kalaiyarasan C', phone: '8248916863' },
    ],
  },
  {
    id: 'ctf',
    name: 'CTF',
    category: 'Technical',
    teamSize: '2–3 Members',
    fee: 200,
    accent: 'cyan',
    image: eventImage,
    imagePosition: '45% 50%',
    format: 'A cybersecurity challenge where teams solve problems and find flags using ethical security techniques.',
    details: [
      'Use ethical and legal methods only.',
      'Do not attack or damage the competition system.',
      'Do not share flags or solutions with other teams.',
      'Do not share hints or write-ups during the event.',
      'Cheating and plagiarism are not allowed.',
      'Do not impersonate another participant.',
      'Brute-force attacks are not allowed unless the challenge permits them.',
      'Respect other participants, organizers, and judges.',
      "Judges' decision is final.",
    ],
    coordinators: [
      { name: 'Sherlin Antolina C', phone: '7200465240' },
      { name: 'Divagar P', phone: '9345488996' },
    ],
  },
  {
    id: 'secure-x-vibecode',
    name: 'Secure X Vibe Coding',
    category: 'Technical',
    teamSize: 'Individual / Up to 2 Members',
    fee: 200,
    accent: 'cyan',
    image: eventImage,
    imagePosition: '52% 38%',
    format: 'Build a useful and working solution based on a problem statement given by the organizers. AI-assisted coding is allowed.',
    details: [
      'Choose one problem statement provided by the organizers.',
      'Build the project during the event.',
      'AI tools such as ChatGPT, Copilot, Cursor, and Gemini are allowed.',
      'Understand your own code and project.',
      'Be ready to explain how you used AI.',
      "Do not copy another participant's project.",
      'Do not submit an old project as your own.',
      'Bring your own laptop and charger.',
      'Maintain professional behaviour.',
      "Judges' decision is final.",
    ],
    coordinators: [
      { name: 'Aravind V', phone: '8825979172' },
      { name: 'Jayadharshini V', phone: '7448680404' },
    ],
  },
  {
    id: 'ppt-ideathon',
    name: 'PPT Ideathon',
    category: 'Technical',
    teamSize: '2–4 Members',
    fee: 200,
    accent: 'cyan',
    image: eventImage,
    imagePosition: '50% 42%',
    format: 'Present one original idea using the official PPT template. Each team gets 5–6 minutes for presentation and 2–3 minutes for Q&A.',
    details: [
      'Use the official PPT template.',
      'Each team can present only one idea.',
      'Team members should be from the same college.',
      'Cross-college teams are not allowed.',
      'The idea must be original.',
      "Do not copy other people's work.",
      'Maintain professional behaviour.',
      'Complete the PPT submission within the given time.',
      'Late submissions are not accepted.',
      "Judges' decision is final.",
    ],
    coordinators: [
      { name: 'Harinisha C', phone: '9655514532' },
      { name: 'Sri R', phone: '8015458886' },
    ],
  },
  {
    id: 'ipl-auction-2026',
    name: 'IPL Auction 2026',
    category: 'Non-technical',
    teamSize: '2–4 Members',
    fee: 150,
    accent: 'lime',
    image: eventImage,
    imagePosition: '54% 48%',
    format: 'A cricket strategy event with two rounds: Rapid Fire and IPL Auction. Teams compete to build a strong 15-player squad using a ₹120 Crore budget.',
    details: [
      'Team must have 2–4 members.',
      'The first round is Rapid Fire.',
      'Top 10 teams qualify for the auction round.',
      'Each team gets ₹120 Crore.',
      'Build a 15-player squad (11 Playing XI + 4 Impact Substitutes).',
      'Maximum 10 Indian and 5 foreign players.',
      'Maximum 4 foreign players can be in the Playing XI.',
      'Do not exceed the ₹120 Crore budget.',
      'No phones, internet, notes, or outside help during the rounds.',
      "Follow the auctioneer's bidding rules.",
      "Judges' decision is final.",
    ],
    coordinators: [
      { name: 'Anisha K', phone: '6369108600' },
      { name: 'Ajith Abhinav RG', phone: '9363569974' },
    ],
  },
  {
    id: 'tune-tracker',
    name: 'Tune Tracker',
    category: 'Non-technical',
    teamSize: '2–4 Members',
    fee: 150,
    accent: 'lime',
    image: eventImage,
    imagePosition: '42% 46%',
    format: 'An audio-based challenge where teams identify songs or audio clues within a fixed time.',
    details: [
      'Team must have 2–4 members.',
      'Report before the event starts.',
      'Only registered team members can participate.',
      'Answer within the given time.',
      'Answers cannot be changed after submission.',
      'Maintain silence while audio clues are played.',
      'Keep phones, smartwatches, and earphones away unless allowed.',
      'Do not use Google, Shazam, AI, or search engines.',
      'Do not communicate with other teams during a round.',
      'Do not record or share the audio clues.',
      'Cheating will lead to disqualification.',
      "Judges' decision is final.",
    ],
    coordinators: [
      { name: 'Surya S P', phone: '9787155806' },
      { name: 'Mirdhula T', phone: '9363974368' },
    ],
  },
  {
    id: 'ai-jail-break',
    name: 'AI Jail Break',
    category: 'Technical',
    teamSize: 'Exactly 2 Members',
    fee: 200,
    accent: 'cyan',
    image: eventImage,
    imagePosition: '48% 44%',
    format: 'A multi-stage AI challenge where teams complete challenges in sequence. Participants progress through Round 1 and Round 2, with scoring based on completed challenges and time used as the tie-breaker.',
    details: [
      'Each team must have exactly 2 members.',
      'Each participant can join only one team.',
      'Team details cannot be changed after registration closes.',
      'Do not use external AI tools such as ChatGPT or other AI assistants.',
      'Complete the rounds in the given order.',
      'Do not skip any stage.',
      'Do not share solutions or flags with other teams.',
      "Do not interfere with another team's session.",
      'Scoring is based on completed challenges.',
      'Time taken will be used as a tie-breaker.',
      'Cheating or rule violations may lead to disqualification.',
      "Judges' and organizers' decision is final.",
    ],
    coordinators: [
      { name: 'Harshit JK', phone: '9952284514' },
      { name: 'Sabeetha D', phone: '9080001039' },
    ],
  },
  {
    id: 'playground-of-hackers',
    name: 'Playground of Hackers',
    category: 'Workshop',
    teamSize: 'Individual',
    fee: 200,
    accent: 'violet',
    image: eventImage,
    imagePosition: '50% 50%',
    format: 'Hands-on offensive & defensive security workshop uncovering real-world exploit vectors, tooling, and live labs.',
    details: [
      'Solo registration',
      'Hands-on practical security & exploitation lab',
      'Laptop recommended with virtualization/browser tools',
    ],
  },
];

export const timeline = [
  ['08:30 AM', 'Registration desk opens', 'Check in, collect your participant confirmation, and move to the inaugural venue.'],
  ['09:00 AM', 'Inauguration', 'Welcome address and Noctivus briefing in the main auditorium.'],
  ['09:30 AM', 'Technical events begin', 'CTF, Bug Bounty 2026, Secure X Vibe Coding, PPT Ideathon, and AI Jail Break move into round one.'],
  ['11:30 AM', 'Non-technical rounds', 'Crime Investigation, Tune Tracker, and IPL Auction 2026 begin from their assigned venues.'],
  ['01:00 PM', 'Lunch break', 'Food counters open according to participant preference and event flow.'],
  ['02:00 PM', 'Final rounds', 'Shortlisted participants move into finals and judging.'],
  ['04:30 PM', 'Valedictory & awards', 'Winners, certificates, and closing ceremony.'],
];

export const brochure = {
  title: "Noctivus '26 Brochure",
  href: '/Noctivus26-brochure.pdf',
  note: 'Download link placeholder. Replace this with the approved PDF before launch.',
};

export const posters = [
  {
    title: 'Demo Cat Image',
    image: '/posters/demo-cat.svg',
    alt: 'Demo cat placeholder image',
  },
  {
    title: 'Demo Dog Image',
    image: '/posters/demo-dog.svg',
    alt: 'Demo dog placeholder image',
  },
  {
    title: 'Demo Cat and Dog',
    image: '/posters/demo-cat-dog.svg',
    alt: 'Demo cat and dog placeholder image',
  },
];

export const crew = [
  ['Faculty Coordinator', 'Department of CSE (Cyber Security)', 'faculty@velammal.edu.in'],
  ['Student Coordinator', 'Noctivus Organizing Team', '+91 98765 43210'],
  ['Registration Desk', 'Payments and confirmations', 'noctivus26@velammal.edu.in'],
];
