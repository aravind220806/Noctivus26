...
export default function App() {
  ...
  return (
    <>
      ...
      {selectedEvent && <EventModal event={selectedEvent} onClose={() => setSelectedEvent(null)} onRegister={() => openRegistration(selectedEvent.id)} />}
      {registration && <RegistrationModal events={events} registrationOpen={registrationOpen} initialEventId={registration === 'open' ? null : registration} onClose={() => setRegistration(null)} />}
    </>
  );
}
    ...
    <section className="hero" id="top">
      ...
      <div className="page-width hero__inner">
        <div className="hero__main">
          ...
          <p className="hero__tagline">Eight events. One charged campus. A symposium built for ideas, instincts, and fearless problem-solving.</p>
  ...
  return (
    <section className="section about" id="about">
      <div className="page-width">
        <SectionTitle kicker="ABOUT NOCTIVUS" title={<>Built for students who<br/><span className="muted-title">want to make things happen.</span></>} />
        <div className="about-grid">
          <div className="about-copy" data-reveal><p className="lead">Noctivus is the annual national-level symposium of the Department of CSE (Cyber Security), Velammal Engineering College.</p><p>Across eight technical and non-technical events, participants solve security problems, build products, test their instincts, and compete through high-energy team challenges.</p></div>
          <div className="about-manifesto" data-reveal><span>WHAT TO EXPECT</span><strong>Practical challenges</strong><strong>Clear judging</strong><strong>Serious competition</strong></div>
        </div>
...
function Schedule() {
  return (
    <section className="section schedule-section" id="schedule">
      <div className="page-width">
        <SectionTitle kicker="SCHEDULE" title="The day is taking shape." description="The official event-day schedule and reporting times will be published after every event format is confirmed." />
        <div className="schedule-reveal" data-reveal>
          <div><span className="schedule-reveal__signal" aria-hidden="true"/><span className="kicker">COMING SOON</span></div>
          <h3>One campus.<br/>Eight challenges.<br/><span>One unforgettable day.</span></h3>
          <p>Technical events, non-technical events, final rounds, and the awards ceremony will all be mapped into one clear schedule before registration opens.</p>
          <a className="button button-secondary" href="#contact">Get official updates <Icon name="arrow" /></a>
        </div>
      </div>
    </section>
  );
...
function Experience() {
  return (
    <section className="section experience-section" id="experience">
      <div className="page-width">
        <SectionTitle kicker="PARTICIPANT INFORMATION" title="Before you arrive." description="Rules, travel information, and official updates will be published here as they are confirmed." />
        <div className="experience-grid">
          <article className="feature-panel feature-panel--brochure" data-reveal><span className="panel-label">EVENT BROCHURE</span><h3>Rules and event formats</h3><p>The official brochure will include complete rules, judging criteria, reporting times, and coordinator details for every event.</p><button className="button button-secondary" disabled>Available soon</button></article>
          <article className="feature-panel feature-panel--transport" data-reveal><span className="panel-label">TRAVEL</span><h3>Campus transport</h3><p>Bus routes and pickup points will be published after transport arrangements are confirmed.</p></article>
          <article className="feature-panel feature-panel--community" data-reveal><span className="panel-label">OFFICIAL UPDATES</span><h3>Participant announcements</h3><p>Registered participants will receive schedule changes and reporting instructions through the official communication channel.</p></article>
        </div>
      </div>
    </section>
  );
    ...
    <section className="section contact-section" id="contact">
      <div className="page-width contact-layout">
        ...
        <div className="contact-card" data-reveal>
          ...
          <p className="placeholder-note">Coordinator contact details are placeholders until the organizing committee confirms them.</p>
        </div>
      </div>
    </section>
  );
...
function EventModal({ event, onClose, onRegister }) {
  ...
  return <div className="modal-shell" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><article className={`event-modal accent-${event.accent}`} role="dialog" aria-modal="true" aria-labelledby="event-modal-title"><button className="icon-button event-modal__close" onClick={onClose} aria-label="Close event details"><Icon name="close"/></button><div className="event-modal__visual"><span>Official event</span><img className="event-modal__emblem" src="/brand/noctivus-emblem.webp" alt="" width="480" height="534"/><small>{event.category}</small></div><div className="event-modal__content"><span className="kicker">EVENT REGISTRATION</span><h2 id="event-modal-title">{event.name}</h2><p className="event-modal__lead">Registration is now open. Complete event format, rules, venue, and timing will be announced separately.</p><div className="rules"><h3>Registration fee</h3><p>{event.category === 'Technical' ? 'Technical event' : 'Non-technical event'} entry · ₹{event.fee} per registration.</p></div><div className="event-modal__action"><div><small>AMOUNT</small><strong>₹{event.fee}</strong></div><button className="button button-primary" type="button" onClick={onRegister}>Register now <Icon name="arrow"/></button></div></div></article></div>;
}
