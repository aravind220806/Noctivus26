    ...
    <div className="modal-shell" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="registration-modal" role="dialog" aria-modal="true" aria-labelledby="registration-title">
        <header className="registration-modal__header">
          <div><span className="kicker">EVENT REGISTRATION</span><h2 id="registration-title">{receipt ? 'Registration received.' : step === 1 ? 'Your details.' : step === 2 ? 'Verify everything.' : 'Complete payment.'}</h2></div>
          <button ref={closeButtonRef} className="icon-button" onClick={onClose} aria-label="Close registration"><Icon name="close" /></button>
        </header>

        {!receipt && <div className="registration-progress" aria-label={`Registration step ${step} of 3`}><span className={step >= 1 ? 'active' : ''}>Details</span><span className={step >= 2 ? 'active' : ''}>Review</span><span className={step >= 3 ? 'active' : ''}>Payment</span></div>}
        ...
        {registrationOpen && !receipt && step === 1 && (
          <div className="registration-body registration-details">
            <div className="selected-registration-event"><label className="field"><span>Event</span><select value={eventId} onChange={(event) => setEventId(event.target.value)}><option value="">Select an event</option>{events.map((item) => <option value={item.id} key={item.id}>{item.name} · ₹{item.fee}</option>)}</select></label>{selectedEvent && <div><span>{selectedEvent.category}</span><strong>₹{amount}</strong></div>}</div>
            ...
            <div className="registration-actions"><button className="button button-primary" type="button" onClick={continueToReview}>Verify details <Icon name="arrow" /></button></div>
          </div>
        )}

        {registrationOpen && !receipt && step === 2 && (
          <div className="registration-body registration-review">
            <div className="review-event"><div><span className="kicker">SELECTED EVENT</span><h3>{selectedEvent?.name}</h3><small>{selectedEvent?.category}</small></div><strong>₹{amount}</strong></div>
            <dl className="review-grid"><ReviewItem label="Full name" value={form.name}/><ReviewItem label="College" value={form.college}/><ReviewItem label="Phone" value={form.phone}/><ReviewItem label="Email" value={form.email}/><ReviewItem label="Food" value={form.foodPreference === 'veg' ? 'Vegetarian' : 'Non-vegetarian'}/></dl>
            ...
            <div className="registration-actions registration-actions--split"><button className="button button-secondary" type="button" onClick={() => setStep(1)}>Edit details</button><button className="button button-primary" type="button" onClick={continueToPayment}>Continue to payment <Icon name="arrow" /></button></div>
          </div>
        )}

        {registrationOpen && !receipt && step === 3 && (
          <form className="registration-body payment-step" onSubmit={submitRegistration}>
            <div className="payment-summary"><div><span>Amount to pay</span><strong>₹{amount}</strong><small>{selectedEvent?.name}</small></div><button type="button" className="text-button" onClick={() => { setPaymentStarted(false); setStep(2); }}>Edit details</button></div>
            {!paymentStarted ? <div className="payment-gate"><span className="kicker">SECURE UPI PAYMENT</span><h3>Ready to pay ₹{amount}?</h3><p>A unique payment QR will be generated for <strong>{selectedEvent?.name}</strong>. Verify the payee and amount in your UPI app before authorizing.</p><div><small>PAYEE</small><strong>{payee}</strong><span>{upiId}</span></div><button className="button button-primary button-large" type="button" onClick={() => setPaymentStarted(true)}>Pay now <Icon name="arrow" /></button></div> : <><div className="payment-layout"><div className="qr-panel">{qrDataUrl ? <img src={qrDataUrl} width="260" height="260" alt={`Dynamic UPI QR code to pay ₹${amount} for ${selectedEvent?.name}`}/> : <div className="qr-loading">Generating payment QR…</div>}<span>Scan & pay with any UPI app</span></div><div className="payment-instructions"><span className="kicker">PAYMENT DETAILS</span><h3>Pay ₹{amount}</h3><dl><div><dt>UPI ID</dt><dd>{upiId}</dd></div><div><dt>Reference</dt><dd>{paymentReference}</dd></div><div><dt>Event</dt><dd>{selectedEvent?.name}</dd></div></dl><a className="button button-primary" href={upiLink}><Icon name="external"/> Open in UPI app</a><p className="payment-safety">Never share your UPI PIN. Complete authorization only inside your trusted UPI app.</p></div></div><label className={`field utr-field utr-field--${utrStatus.state}`}><span>12-digit UTR / payment reference</span><input type="text" inputMode="numeric" pattern="[0-9]{12}" minLength="12" maxLength="12" autoComplete="off" placeholder="Enter 12 digits after payment" value={utr} aria-invalid={utrStatus.state === 'duplicate'} aria-describedby="utr-status" onChange={(event) => { setError(''); setUtr(event.target.value.replace(/\D/g, '').slice(0, 12)); }}/><small id="utr-status" className="utr-status" aria-live="polite">{utrStatus.message}</small></label><label className="check-field"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)}/><span>I confirm that I paid the exact amount and consent to these details being stored for event registration and payment verification.</span></label>{error && <p className="form-error" role="alert">{error}</p>}<div className="registration-actions registration-actions--split"><button className="button button-secondary" type="button" onClick={() => setStep(2)}>Back</button><button className="button button-primary" disabled={submitting || utr.length !== 12 || utrStatus.state === 'checking' || utrStatus.state === 'duplicate'} type="submit">{submitting ? 'Submitting…' : utrStatus.state === 'checking' ? 'Checking UTR…' : 'Submit registration'} <Icon name="arrow" /></button></div></>}
          </form>
        )}

        {registrationOpen && receipt && <div className="registration-body receipt-step"><div className="success-mark"><Icon name="check" size={34}/></div><span className="kicker">PAYMENT VERIFICATION PENDING</span><h3>You're registered.</h3><p>Your entry has been received. Organizers will verify the UTR before confirming the registration.</p><div className="receipt-card"><span>Registration ID</span><strong>{receipt.registrationId}</strong><small>Save this number for future communication.</small></div><button className="button button-primary" type="button" onClick={onClose}>Return to Noctivus <Icon name="arrow" /></button></div>}
      </section>
    </div>
  );
}
