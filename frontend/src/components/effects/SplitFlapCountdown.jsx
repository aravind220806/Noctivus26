...
export default function SplitFlapCountdown({ target }) {
  ...
  useEffect(() => {
    const update = () => setClock((value) => ({ previous: value.current, current: calculate(target) }));
  ...
  if (clock.current.total === 0) return <div className="countdown-live"><span /> The signal is live</div>;

  return (
    <div className="hero-countdown" aria-label={`${clock.current.days} days, ${clock.current.hours} hours, ${clock.current.minutes} minutes, and ${clock.current.seconds} seconds until Noctivus`}>
      <span className="hero-countdown__eyebrow" aria-hidden="true"><i /> COUNTDOWN TO NOCTIVUS '26 <i /></span>
