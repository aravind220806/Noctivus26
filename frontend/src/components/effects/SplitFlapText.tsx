...
const resolveCharset = (charset?: string): string =>
  (charset && CHARSETS[charset]) || (typeof charset === 'string' && charset ? charset : CHARSETS.alphanumeric);
const normalizePhrase = (phrase: string | null | undefined, width: number): string =>
...
const buildSequence = (target: string, flips: number, charset: string): string[] =>
  Array.from({ length: flips }).map(() => sampleChar(charset)).concat(target);
...
export default function SplitFlapText({
  ...
  useEffect(() => {
    ...
    const animateTo = (targetPhrase: string): number => {
      ...
      const plans: AnimationPlan[] = targetPhrase
        .split('')
        .map((target, index) => {
          ...
          return from === target
            ...
            : {
                ...
                from,
                target,
                sequence: buildSequence(target, flipCount, activeCharset),
                start: index * staggerMs,
                step: -1,
                done: false,
              };
        })
        .filter((item): item is AnimationPlan => item !== null);
